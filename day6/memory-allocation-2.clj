...
(defn run-iteration
  ...
  (let [updatedVector    (updateVector currentVector (getMax currentVector))
    ...
    (if (contains? existingVectors leftSortedVector)
      (- iteration (get existingVectors leftSortedVector))
      #(run-iteration
        (assoc existingVectors leftSortedVector iteration)
        (inc iteration)
        updatedVector))))

(trampoline run-iteration (hash-map) 0 [11 11 13 7 0 15 5 5 4 4 1 1 7 1 15 11])
