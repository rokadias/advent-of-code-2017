...
(defn- getChildNames
  [childNamesString]
  (when childNamesString (str/split (subs (str/trim childNamesString) 3) #", ")))

(defn- associateChildWithParent
  [map childName parentName]

  (assoc map childName
         (if (contains? map childName)
           (assoc (get map childName) :parent parentName)
           { :name name :parent parentName })))

(defn parseCommand
  [map command]
  (let [values   (re-find #"(\S+) \((\d+)\)( -> .*)?" command)
        name     (get values 1)
        weight   (Integer/parseInt (get values 2))
        children (get values 3)
        childNames (getChildNames children)
        parent (get-in map [name :parent])
        additionalChildrenMap (reduce #(associateChildWithParent %1 %2 name) map childNames)]

    (assoc additionalChildrenMap name
           {
            :name name
            :weight weight
            :children childNames
...
(defn findImbalancedWeight
  [weights]

  (if (= 1 (count (dedupe weights)))
    nil
    (let [initial (first weights)
          initialCount (reduce #(if (= initial %2) (inc %1) %1) 0 weights)
          differentValue (some #(when (not= initial %1) %1) (rest weights))
          differentIndex (first (keep-indexed #(when (= differentValue %2) %1) weights))]
      (println differentIndex initial differentValue)
      (if (= initialCount 1)
        [0 (- differentValue initial)]
        [differentIndex (- initial differentValue)]
        )
      )
    )
  )

(defn findIncorrectWeight
  [map parentName]

  (let [nodeMap (get map parentName)
        children (get nodeMap :children)
        weight (get nodeMap :weight)]
    (if (nil? children)
      [weight]
      (let [childrenWeights (mapv #(findIncorrectWeight map %1) children)
            incorrectWeight (some #(when (not (vector? %1)) %1) childrenWeights)]
        (if incorrectWeight
          incorrectWeight
          (let [sums (mapv #(reduce + 0 %) childrenWeights)
                imbalancedWeight (findImbalancedWeight sums)
                index (first imbalancedWeight)
                delta (second imbalancedWeight)]
            (if (nil? imbalancedWeight)
              (apply conj [weight] sums)
              (+ (first (get childrenWeights index)) delta)
              )
            )
          )
        )
...
(def treeMaped (reduce #(parseCommand %1 %2) (hash-map) commands))

(def root (map #(get %1 :name) (filter #(nil? (get %1 :parent)) (vals treeMaped))))
(findIncorrectWeight treeMaped (first root))
